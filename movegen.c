u64 getRayBB(u64 occ, u8 dir, u8 pos) {
	...
	//bbprint(blockers);
	if (blockers != 0) {
		int c = bitScan(blockers, dir == 6 || dir == 7 || dir == 0 || dir == 1);
...
void genLegalMoves(Board* b, List* l) {
	for (int p = 0; p < 6; p++) { // for all piece types
		...
		//for (u8 i = (u8)ctz((u64)p); i < (64 - clz((u64)p)); i++) { // iterate over them
		for (u8 i = 0; i < 64; i++) { // iterate over them
...
bool leavesInCheck(Board* old, Move* m) {
	...
	bool heclr = !b->color;
	u64 hepiece = b->pieces[heclr][PAWN]; if (pawnAttacks[b->color][myKing] & hepiece) { dfree(b); return true; }
	for (int i = 1; i < 6; i++) { hepiece = b->pieces[heclr][i]; if (genMovesBB[i](myKing, b) & hepiece) { dfree(b); return true; } }
	dfree(b);
	return false;
}
